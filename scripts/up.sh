...
deploy_aws() {
  ...
  docker buildx build \
    ...
    --tag 565273632019.dkr.ecr.us-east-1.amazonaws.com/debugpilot-api:latest \
    --push .

  echo "🔀 Switching kubectl to AWS..."
  kubectl config use-context Debugpilot@debugpilot.us-east-1.eksctl.io

  echo "⎈ Deploying with Helm to AWS..."
  helm upgrade --install debugpilot ./charts/debugpilot \
    --set api.image=565273632019.dkr.ecr.us-east-1.amazonaws.com/debugpilot-api:latest \
    --set api.replicas=1

  echo "⏳ Waiting for pods..."
  kubectl wait --for=condition=ready pod \
    -l app=debugpilot-api --timeout=120s
...
deploy_gcp() {
  ...
  if gcloud container clusters describe debugpilot \
    ...
    gcloud container clusters create debugpilot \
      --region=us-central1 \
      --num-nodes=2 \
  ...
  docker buildx build \
    --platform linux/amd64,linux/arm64 \
    --tag "us-central1-docker.pkg.dev/${GCP_PROJECT}/debugpilot/debugpilot-api:latest" \
    --push .
  ...
  helm upgrade --install debugpilot ./charts/debugpilot \
    --set "api.image=us-central1-docker.pkg.dev/${GCP_PROJECT}/debugpilot/debugpilot-api:latest" \
    --set api.replicas=1
